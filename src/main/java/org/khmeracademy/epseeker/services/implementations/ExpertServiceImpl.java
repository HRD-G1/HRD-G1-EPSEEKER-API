...
public class ExpertServiceImpl implements ExpertService {
	...
	@Override
	public ArrayList<Expert> findAll(Pagination pagination) {
		try {
			pagination.setTotalCount(expertRepository.findAllAndCount());
			int limit = pagination.getLimit();
			int offset = pagination.getOffset();
			System.out.println(expertRepository.findAll(limit, offset));
			return expertRepository.findAll(limit, offset);
		} catch (Exception e) {
	...
	public boolean save(Expert exp) throws Exception {
		...
		try {
			...
			for (int i = 0; i < exp.getExpertLanguageDetail().size(); i++) {
				...
				try {
					if(exp.getExpertLanguageDetail().get(i).getMention().equals("Low")){
						exp.getExpertLanguageDetail().get(i).setLevelNumber(25);
					}else if(exp.getExpertLanguageDetail().get(i).getMention().equals("Medium")){
						exp.getExpertLanguageDetail().get(i).setLevelNumber(50);
					}else{
						exp.getExpertLanguageDetail().get(i).setLevelNumber(75);
					}
			...
			for (int i = 0; i < exp.getExpertDocuments().size(); i++) {
				...
				try {
					System.out.println(exp.getExpertDocuments().get(i));
